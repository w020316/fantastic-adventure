/**
 * 通过 API 创建技术文章
 * 运行方式: npx tsx scripts/seed-articles-api.ts
 * 需要先设置环境变量: ADMIN_EMAIL, ADMIN_PASSWORD, SITE_URL
 */

const SITE_URL = process.env.SITE_URL || 'https://fantastic-adventure.fly.dev'
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'admin@xiaowu.dev'
...
async function main() {
  console.log(`🚀 开始通过 API 创建文章...\n站点: ${SITE_URL}\n`)

  // 1. 通过 NextAuth credentials 回调登录 (form-encoded)
  const loginRes = await fetch(`${SITE_URL}/api/auth/callback/credentials`, {
    method: 'POST',
  ...
  if (!loginRes.ok) {
    console.error('❌ 登录失败，请检查 ADMIN_EMAIL 和 ADMIN_PASSWORD')
    process.exit(1)
  }
  ...
  console.log(`Session: ${JSON.stringify(sessionData).substring(0, 200)}`)

  // 2. 获取现有标签
  const tagsRes = await fetch(`${SITE_URL}/api/tags`)
  const tagsData = await tagsRes.json()
  const existingTags = new Map<string, string>(
    tagsData.tags.map((t: any) => [t.slug, t.id])
  )

  console.log(`✅ 已有标签: ${existingTags.size}`)
  ...
  for (const article of articles) {
    // 检查是否已存在
    try {
      const checkRes = await fetch(`${SITE_URL}/api/articles/${article.slug}`)
      if (checkRes.ok) {
        console.log(`⏭️  跳过(已存在): ${article.title}`)
        skipped++
        continue
      }
    } catch {
    ...
    const tagIds = article.tagSlugs
      .map(slug => existingTags.get(slug))
      .filter(Boolean) as string[]

    // 创建文章
    const createRes = await fetch(`${SITE_URL}/api/articles`, {
      method: 'POST',
      headers: {
    ...
    if (createRes.ok) {
      ...
      console.error(`❌ 创建失败: ${article.title} - ${err.error || '未知错误'}`)
    }
