...
async function main() {
  ...
  const articles = [
    // ============ 全栈方向 ============
    {
      ...
      excerpt:
        '基于 Next.js 16 App Router + Prisma 6 + Tailwind CSS 4 构建的个人作品集与博客系统，采用赛博朋克设计语言（霓虹色、故障艺术、Orbitron 字体）。涵盖文章管理、评论系统、点赞收藏、项目管理、联系表单、CMS 后台、AI 助手、音乐播放器等模块，部署于 Fly.io 香港节点，Lighthouse 90+ 分，是设计与工程能力的综合体现。',
...
> 全栈开发的核心不是掌握所有技术，而是理解各层级的协作方式。`,
    ...
    },
    {
      ...
      categorySlug: 'fullstack',
      excerpt:
        '面向高校师生的校园信息服务平台，核心解决教材信息查询困难、教学日历不透明、校园资讯分散三大痛点。基于 Vue 3.5 + Vite 6 + Pinia + Vercel Serverless + Prisma + Neon PostgreSQL 构建，含教材多维度查询、校历教学周、新闻资讯、用户系统、积分、消息通知六大模块及独立管理后台，已迭代至 v12.0，含 13+ 数据模型与 11+ API 模块。',
...
> 将分散信息整合为统一平台，是产品思维的实战训练。`,
    ...
    // ============ AI项目方向 ============
    {
      ...
      excerpt:
        '基于多智能体协作的 AI 日记应用，通过情绪感知器、记忆管家、日记生成器、对话精灵四个智能体分工协作，实现情绪识别、记忆管理、个性化日记生成与温暖对话。技术栈含 Python + FastAPI + DeepSeek API + ChromaDB + BGE 嵌入模型 + SSE 实时推送，支持向量记忆检索与在线体验，是 AI 落地产品的完整实践。',
...
> 多智能体系统让 AI 应用更加智能和个性化。`,
    ...
    },
    {
      ...
      categorySlug: 'ai-projects',
      excerpt:
        '基于 YOLO 目标检测算法的食品安全检测系统，实时识别食品加工人员是否佩戴安全装备（头盔、口罩、手套、围裙、靴子）及是否存在违规操作。涵盖训练数据集准备、模型训练（mAP 85%+）、Web 检测界面开发及 GitHub Pages 部署，支持实时 30+ FPS 检测，是计算机视觉从训练到部署的全流程实践。',
...
> 计算机视觉落地为可用产品，需要工程化思维。`,
    ...
    // ============ 前端方向 ============
    {
      ...
      excerpt:
        '基于 React + D3.js + WebSocket 构建的数据可视化仪表盘，支持实时数据流展示、自定义图表配置与智能预警。通过 WebSocket 实现数据实时推送，D3.js 绘制可交互图表，并提供灵活的配置面板让用户自定义仪表盘布局，是前端数据可视化与实时通信的综合实践。',
...
> 数据可视化让复杂数据变得直观可懂。`,
    ...
    },
    {
      ...
      categorySlug: 'frontend',
      excerpt:
        '基于 TypeScript + Canvas API + Web Workers 构建的像素艺术生成器，支持多种生成算法、调色板自定义与导出功能。利用 Web Workers 将计算密集的生成任务放到后台线程，保证 UI 流畅；Canvas API 实现像素级绘制与实时预览。作为个人博客作品集系统的子模块，体现了前端工程化与性能优化能力。',
...
> 前端不只是展示，也能做计算密集型任务。`,
    ...
    },
    {
      ...
      categorySlug: 'frontend',
      excerpt:
        '基于 HTML + JavaScript + CSS + RESTful API 实现的前后端分离学生信息管理系统，支持学生信息的增删改查、分页查询、条件搜索。采用前后端分离架构，前端纯静态实现部署于 GitHub Pages，后端提供 RESTful API，是入门全栈开发的经典实践项目。',
...
> 从简单项目入手，理解全栈基本架构。`,
    ...
    },
    {
      ...
      categorySlug: 'frontend',
      excerpt:
        '基于 HTML + JavaScript + CSS + Swagger 构建的电商订单查询系统，支持订单状态跟踪、多条件搜索、订单详情查看。提供完整的 Swagger API 文档，便于接口联调与维护。纯前端实现并部署于 GitHub Pages，是 API 文档规范化的实践项目。',
      content: `# 电子订单查询系统
...
> 规范的 API 文档是团队协作的基础。`,
    ...
    },
    {
      ...
      categorySlug: 'frontend',
      excerpt:
        '基于 JavaScript + HTML + CSS 构建的旅游攻略平台，提供景点推荐、路线规划、攻略分享等功能。采用响应式设计，纯前端实现并部署于 GitHub Pages，注重移动端体验与视觉表现力，是前端工程与设计审美的综合实践。',
      content: `# 旅游攻略网站
...
> 前端不只是代码，更是体验设计。`,
    ...
    // ============ Java后端方向 ============
    {
      ...
      excerpt:
        '基于 Spring Boot + Vue + MySQL + Element UI 构建的企业管理后台系统，涵盖员工管理、部门管理、权限控制等核心模块。采用前后端分离架构，后端 Spring Boot 提供 RESTful API，前端 Vue + Element UI 实现，部署于 GitHub Pages。是 Java 后端工程化与企业级应用开发的综合实践。',
...
> Java 后端的工程化是企业级应用的基础。`,
    ...
    },
    {
      ...
      categorySlug: 'java-backend',
      excerpt:
        '基于 Java + Maven + Tomcat + MySQL + JSP 构建的学生学籍信息管理系统，含学生信息管理、课程流程、数据统计等功能模块。采用经典 Java Web 技术栈（Servlet + JSP），通过 Maven 管理依赖，部署于 GitHub Pages。是 Java Web 基础与传统开发流程的实践项目。',
